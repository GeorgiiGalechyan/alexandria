...
Расширяет (extends) rкласс [`<EventEmitter>`](https://nodejs.org/api/events.html#class-eventemitter)
...
> **Примечание переводчика:**  
> При описании событий (events) мы будем использовать выражения **_"генерирация события"_**, **_"излучение или испускание сигнала"_**, **_"сигнализирование о событии"_**. Напимер, применительно к классу `net.Server`, когда мы будем говорить, что при закрытии сервера генерируется событие `'сlose'`, то будем иметь в виду, что при закрытии сервера по тем или иным причинам (плановое закрытие, разрыв соединения, ошибка программы и т.д.), будет вызван код (напр. функция), который "подписан" на событие `'close'`. Таким образом, прописывая код и подписывая его не событие `'close'` мы можем указать, как должно вести себя приложение (какой код должен сработать) при закрытии сервера. Таким образом, мы можем спрограмировать поведение приложения в зависимости от причины закрытия сервера, прописав соответствующую логику.
...
Событие `'close'` генерируется при закрытии сервера. Если соединения существуют, это событие **не генерируется до тех пор**, пока не будут завершены все соединения.
...
- [`<Error>`]() - объект к которому подключаются.
...
> **Примечание переводчика:**  
> Нужно различать классы `net.Server` (сервер) и `net.Socket` (сокет). Ошибки могут происходить и на уровне сервера, и на уровне сокета внутри этого сервера. События `'error'` и `'close'` генерирются по разному в зависимости от экземпляра (instance) класса в котором они возникли. Например, событие `error` у класса `net.Socket` **автоматически вызовет событие** `'close'` у класса `net.Socket`.  
...
Событие `'drop'` генерируется когда количество подключений достигает порога параметра `server.maxConnections`. По достижении порого, сервер отбрасывает (drop) новые подключения и вместо этого генерирует событие `'drop'`. Если наш экземпляр сервера работает по протоколу TCP (_TCP-сервер_), то аргумент выглядит следующим образом (см. ниже), в противном случае аргумент не определён (**`undefined`**).
