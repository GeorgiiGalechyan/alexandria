...
Встроенный модуль Net предоставляет набор API для **асинхронной** работы с сетями, для создания потоковых (stream-based) серверов TCP или IPC, а так же клиентов.
...
Объект `BlockList` можно передавать в параметры некоторых сетевых API для указания правил отключения входящего или исходящего доступа к определенным IP-адресам, диапазонам IP-адресов или подсетям IP.
...
- **`type:`** [`<string>`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) - значение либо `ipv4` or `ipv6`. **Default:** `ipv4`.
...
```
  ...
  blockList.addAddress('28.0.178.165', 'ipv4')

  // аналогично предыдущей записи
  blockList.addAddress('28.0.178.165')  // type = ipv4
```
...
- **`end`:** [`<string>`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) | [`<net.SocketAddress>`]() - конечный адрес формата IPv4 или IPv6 в диапазоне IP адресов.
- **`type:`** [`<string>`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#String_type) - значение либо `ipv4` or `ipv6`. **Default:** `ipv4`.
...
> **Примечание переводчика:**  
> Так же рекоммендую ознакомиться с маленькой русскоязычной статьёй на Wikipedia - **["Сетевой адрес"](https://ru.wikipedia.org/wiki/%D0%A1%D0%B5%D1%82%D0%B5%D0%B2%D0%BE%D0%B9_%D0%B0%D0%B4%D1%80%D0%B5%D1%81)**. данная статья нужна чтобы на базом уровне понять, что такой **_"адрес сети"_**, а что такое **_"сетевой адрес"_**.
...
Возвращает `true`, если данный IP-адрес соответствует любому из правил, добавленных в черный список., т.е. проверяет, находится ли указанный IP адресс в `BlockList`.
